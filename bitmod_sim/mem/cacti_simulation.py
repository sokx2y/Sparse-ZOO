...
class CactiSimulation:
    ...
    ## Some CACTI builds emit the output next to the binary cwd instead of next to infile.
    CACTI_OUTPUT_FILE_FALLBACK = os.path.join(CACTI_PROGRAM_PATH, 'cache.cfg.out')
    ...
    def __init__(self, mem_config):
        ...
        for output_file in (self.CACTI_OUTPUT_FILE, self.CACTI_OUTPUT_FILE_FALLBACK):
    ...
    def run_cacti(self):
        ...
        #################### 3. Collect results ####################
        output_file = self.CACTI_OUTPUT_FILE
        if not os.path.exists(output_file) and os.path.exists(self.CACTI_OUTPUT_FILE_FALLBACK):
            output_file = self.CACTI_OUTPUT_FILE_FALLBACK
        if not os.path.exists(output_file):
            stderr_msg = stderr.decode(errors='replace').strip()
            stdout_msg = stdout.decode(errors='replace').strip()
            raise RuntimeError(
                'CACTI did not generate cache.cfg.out. '
                f'returncode={stream.returncode}, '
