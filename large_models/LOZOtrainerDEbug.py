...
# 
    ...
    def _fd_is_expected_frozen_param(self, param_name: str) -> bool:
        """
        In trainable_mode=linear_only, embedding/norm/lm_head are intentionally frozen.
        forward_delta modules may still call providers for them, so provider should return zero delta.
        """
        mode = (getattr(self.args, "trainable_mode", "all") or "all").lower()
        if mode != "linear_only":
            return False
        ...
        return any(keyword in name for keyword in frozen_keywords)

    def _fd_zero_z_for_frozen(self, param_name, shape, device, dtype):
        if not hasattr(self, "_fd_zero_delta_seen"):
            self._fd_zero_delta_seen = set()

        key = ("z", param_name)
        if key not in self._fd_zero_delta_seen:
            print(
                f"[FD_PROVIDER_ZERO][z][linear_only] param_name={param_name}, shape={shape}",
                flush=True,
            )
            self._fd_zero_delta_seen.add(key)

        z = torch.zeros(shape, device=device, dtype=dtype)
        scale = 0.0
        return z, scale

    def _fd_zero_uv_for_frozen(self, param_name, shape, device, dtype):
        if not hasattr(self, "_fd_zero_delta_seen"):
            self._fd_zero_delta_seen = set()
        ...
        if key not in self._fd_zero_delta_seen:
            print(
                f"[FD_PROVIDER_ZERO][uv][linear_only] param_name={param_name}, shape={shape}",
                flush=True,
            )
            self._fd_zero_delta_seen.add(key)
        ...
        if len(shape) != 2:
            raise RuntimeError(
                f"[FD_PROVIDER_ZERO][uv] expected 2D weight shape for frozen param, "
                f"but got param_name={param_name}, shape={shape}"
            )

        rank = int(getattr(self.args, "rank_r", 1))
        u = torch.zeros((shape[0], rank), device=device, dtype=dtype)
        v = torch.zeros((shape[1], rank), device=device, dtype=dtype)
        scale = 0.0
        return u, v, scale
    ...
    def make_z_provider(self):
        ...
        def provider(param_name, shape, device, dtype, inference_count):
            if not hasattr(self, "z") or param_name not in self.z:
                if self._fd_is_expected_frozen_param(param_name):
                    return self._fd_zero_z_for_frozen(param_name, shape, device, dtype)
    ...
    def make_uv_provider(self):
        ...
        """
        def provider(param_name, shape, device, dtype, inference_count):
            missing_u = (not hasattr(self, "u")) or (param_name not in self.u)
            missing_v = (not hasattr(self, "v")) or (param_name not in self.v)

            if missing_u or missing_v:
                if self._fd_is_expected_frozen_param(param_name):
                    return self._fd_zero_uv_for_frozen(param_name, shape, device, dtype)
    ...
    def _debug_phase1_write_jsonl(self, row):
    ...
    def _debug_phase1_compare_step(self, model, inputs):
        ...
        loss_plus_explicit_f = float(loss_plus_explicit.detach().float().cpu().item())
        loss_minus_explicit_f = float(loss_minus_explicit.detach().float().cpu().item())
        loss_plus_fd_f = float(loss_plus_fd.detach().float().cpu().item())
        loss_minus_fd_f = float(loss_minus_fd.detach().float().cpu().item())
        projected_grad_explicit_f = float(projected_grad_explicit.detach().float().cpu().item())
        projected_grad_fd_f = float(projected_grad_fd.detach().float().cpu().item())
        ...
        rel_projected_grad = abs_projected_grad / (abs(projected_grad_explicit_f) + 1e-12)
        first_dtype = None
        ...
        row = {
            ...
            "exact_effective_delta": bool(getattr(self.args, "debug_fd_exact_effective_delta", False)),
            "model_param_dtype": first_dtype,
        }
        self._debug_phase1_write_jsonl(row)
        print(
            "[PHASE1] "
            f"step={row['step']} "
            f"seed={row['zo_random_seed']} "
            f"exact={row['exact_effective_delta']} "
