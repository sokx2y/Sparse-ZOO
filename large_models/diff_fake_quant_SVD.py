...
SVD_LORA_RANK_GROUP_SIZE = 16


def _quantize_svd_lora_rank_nvfp8(rank_values: torch.Tensor):
    """NVFP8-quantize rank values in groups of 16 with logical zero padding."""
    rank = rank_values.shape[-1]
    if rank == 0:
        return rank_values
    padded_rank = ((rank + SVD_LORA_RANK_GROUP_SIZE - 1) // SVD_LORA_RANK_GROUP_SIZE) * SVD_LORA_RANK_GROUP_SIZE
    padding = padded_rank - rank
    if padding:
        rank_values = F.pad(rank_values, (0, padding))
    quantized = quantize_activation_nvfp8(
        rank_values,
        group_size=SVD_LORA_RANK_GROUP_SIZE,
    )
    return quantized[..., :rank]


def _q_svd_lora_output_from_rank_values(
    rank_values: torch.Tensor,
    u: torch.Tensor,
    input_shape,
):
    quantized_rank_values = _quantize_svd_lora_rank_nvfp8(rank_values)
    out = quantized_rank_values.matmul(u.t())
    return out.view(*input_shape[:-1], u.shape[0])


def q_svd_lora_output(input: torch.Tensor, u: Optional[torch.Tensor], v: Optional[torch.Tensor]):
    if u is None or v is None:
        return None
    flat = input.reshape(-1, input.shape[-1]).to(dtype=v.dtype)
    rank_values = flat.matmul(v)
    return _q_svd_lora_output_from_rank_values(rank_values, u, input.shape)


def _q_svd_lora_combined_output(
    input: torch.Tensor,
    ...
    flat = input.reshape(-1, input.shape[-1]).to(dtype=v.dtype)
    diff_flat = diff_input.reshape(-1, diff_input.shape[-1]).to(dtype=v.dtype)
    rank_values = flat.matmul(v) + diff_flat.matmul(v)
    return _q_svd_lora_output_from_rank_values(rank_values, u, input.shape)
...
def q_svd_lora_forward_delta(module, input, diff_input):
    raw_u = getattr(module, "svd_lora_u", None)
    raw_v = getattr(module, "svd_lora_v", None)
    ...
    raw_u_minus = raw_u if du is None else raw_u + u_scale * du
    raw_v_minus = raw_v if dv is None else raw_v + v_scale * dv
    quantize_factors = getattr(module, "quantize_svd_lora_factors_fp8", False)
    if quantize_factors:
        u = quantize_svd_lora_factor_nvfp8(raw_u)
        v = quantize_svd_lora_factor_nvfp8(raw_v)
        # Quantize the perturbed factors after addition so the operands used by
        # the updated SVD branch are Q(U+dU) and Q(V+dV).
        u_minus = quantize_svd_lora_factor_nvfp8(raw_u_minus)
        v_minus = quantize_svd_lora_factor_nvfp8(raw_v_minus)
    else:
        u, v = raw_u, raw_v
        u_minus, v_minus = raw_u_minus, raw_v_minus

    if getattr(module, "debug_nonfinite", False):
        quantized_factors = (u, v, u_minus, v_minus)
        if (
            any(t is not None and not torch.isfinite(t).all() for t in quantized_factors)
            and not getattr(module.__class__, "_printed_factor_nonfinite_debug", False)
        ):
            ...
            print(
                ...
                f"layer={module.layer_name}",
                f"inference_count={module.inference_count}",
                _finite_stats("u", raw_u),
                _finite_stats("v", raw_v),
                _finite_stats("du", du),
                _finite_stats("dv", dv),
                _finite_stats("u_q", u),
                _finite_stats("v_q", v),
                _finite_stats("u_minus_q", u_minus),
                _finite_stats("v_minus_q", v_minus),
                flush=True,
            )

    plus = q_svd_lora_output(input, u, v)
    if plus is None:
        return None, None

    # Fuse the two V-side projections before their single NVFP8 quantization.
    # This remains the exact QDiffSVD path even when appro_SVD is set.
    minus = _q_svd_lora_combined_output(input, diff_input, u_minus, v_minus)
    ...
    if svd_term_profile_enabled(module.inference_count):
        ...
        delta_v = v_minus - v
        terms = {
            "x_dv_u": q_svd_lora_output(input, u, delta_v),
            "x_v_du": q_svd_lora_output(input, delta_u, v),
            "x_dv_du": q_svd_lora_output(input, delta_u, delta_v),
            "dx_v_u": q_svd_lora_output(diff_input, u, v),
            "dx_dv_u": q_svd_lora_output(diff_input, u, delta_v),
            "dx_v_du": q_svd_lora_output(diff_input, delta_u, v),
            "dx_dv_du": q_svd_lora_output(diff_input, delta_u, delta_v),
        }
        approximations = {
            "first_order": terms["x_dv_u"] + terms["x_v_du"] + terms["dx_v_u"],
...
class QdiffSVDLinear(QdiffLinear):  # PORTED: inherit OPT QdiffLinear so SVD replacement remains structurally compatible.
    ...
    def forward_delta(self, input: torch.Tensor, diff_input: torch.Tensor):
        ...
        output = self._base_linear(input_q, self.weight, self.bias)
        svd_output, svd_diff = q_svd_lora_forward_delta(self, input_q, diff_input_q)
        if svd_output is not None:
            output = output + svd_output
