...
import argparse
import time
import sys  # PORTED: SmoothSVD adds SVD-ZOO-Quant to sys.path at runtime.
from pathlib import Path  # PORTED: SmoothSVD checkpoint/activation-scale path handling.
import tasks
...
from nv_quant_linear import replace_opt_linears_with_nv
...
class OurArguments(TrainingArguments):
    ...
    debug_device_preflight: bool = field(  # PORTED: optional multi-GPU/device-map smoke test before training.
        default=False,
        metadata={"help": "Run one train forward_delta batch and one eval normal-forward batch before training."}
    )
    debug_device_preflight_only: bool = field(  # PORTED: allow checking multi-GPU placement without starting training.
        default=False,
        metadata={"help": "Run device preflight and exit without training."}
    )
    debug_loss: bool = field(
        default=False,
        metadata={"help": "Print high-precision LOZO loss1/loss2/projected_grad diagnostics for early steps."}
    )
    ...
    # SmoothQuant + SVD-LoRA compensation. Disabled by default.  # PORTED: expose SmoothSVD path without changing old runs.
    ...
    appro_SVD: bool = field(  # PORTED: expose llama3 approximate SVD forward_delta switch for large_model scripts.
        default=False,
        metadata={
            "help": (
...
class Framework:
    ...
    def _add_svd_zoo_to_path(self):  # PORTED: SmoothSVD depends on sibling SVD-ZOO-Quant package.
    ...
    def _svd_lora_quant_settings(self):  # PORTED: normalize runtime quant format for SVD replacement and attachment.
    ...
    def _prepare_svd_lora_compensation(self, model, tokenizer):  # PORTED: apply SmoothQuant and load SVD-LoRA factors before diff replacement.
        ...
        logger.info("[SVD-LoRA] checkpoint compensation_mode=%s quant_format=%s", mode, self.svd_lora_metadata.get("quant_format"))
    ...
    def get_max_memory(self, reserve_gb=6, low0_extra_reserve_gb=8):  # PORTED: multi-GPU device_map memory budget helper.
        max_memory = {}
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            ...
            if self.args.head_tuning:
                ...
                max_memory = self.get_max_memory(reserve_gb=5, low0_extra_reserve_gb=0)  # PORTED: share multi-GPU memory budgeting with SmoothSVD path.
                logger.info(f"[DEVICE_MAP] max_memory = {max_memory}")  # PORTED: debug multi-card placement.
                logger.info(f"[DEVICE_MAP] visible cuda devices = {torch.cuda.device_count()}")  # PORTED: debug multi-card placement.
                if is_llama:
                    model = AutoModelForCausalLM.from_pretrained(
                        model_source,
                        config=config,
                        ...
                        max_memory=max_memory,  # PORTED: use per-GPU budget helper.
            ...
            logger.info(f"[DEVICE_MAP] hf_device_map = {getattr(model, 'hf_device_map', None)}")  # PORTED: debug multi-card placement.
            if getattr(config, "model_type", None) == "opt":  # PORTED: OPT-specific device boundary logging.
                logger.info(f"[DEVICE_MAP] embed_tokens device = {model.model.decoder.embed_tokens.weight.device}")
        ...
        self.svd_lora_state_dict = self._prepare_svd_lora_compensation(model, tokenizer)  # PORTED: SmoothSVD smoothing happens before diff-layer replacement.
    ...
    def train(self, train_samples, eval_samples):
        ...
        if nv_quant_mode != "none":
            ...
            logger.info(
                "Replaced %d OPT decoder Linear modules for NV %s quantization",
                replaced,
                nv_quant_mode.upper(),
            )
        ...
        if self.args.apply_forward_delta:
            ...
            if self.args.svd_lora_compensation:  # PORTED: load OPT SVD-aware replacement and factor attachment only when enabled.
            ...
            if self.model.config.model_type == "opt":
                if self.args.svd_lora_compensation:  # PORTED: SmoothSVD OPT path replaces linears with SVD-aware diff modules.
                    QuantizeOPTForLOZOSVD(
                        model=self.model,
                        uv_provider=uv_provider,
                        z_provider=z_provider,
                        quant_format=self.args.svd_lora_quant_format,
                        quant_format_wdx=self.args.quant_format_wdx,
                ...
                raise NotImplementedError(  # PORTED: large_model is OPT-specific; avoid carrying LLaMA replacement code.
                    f"apply_forward_delta is not implemented for model_type={self.model.config.model_type}"
                )

            if self.args.svd_lora_compensation:  # PORTED: attach offline SVD-LoRA factors after SVD-aware layer replacement.
                ...
                if self.args.appro_SVD:  # PORTED: match llama3_model approximate SVD delta toggle after factors are attached.
                    print("[SVD-LoRA] enable approximate SVD forward_delta")  # PORTED: visible confirmation for script logs.
                    for module in self.model.modules():  # PORTED: only SVD branches with attached factors can use the approximation.
                        if hasattr(module, "svd_lora_u") and hasattr(module, "svd_lora_v"):  # PORTED: avoid touching SVD-aware layers without factors.
                            module.appro_SVD = True  # PORTED: svd_lora_forward_delta reads this flag.
        ...
        if self.args.debug_device_preflight or self.args.debug_device_preflight_only:  # PORTED: optional multi-GPU/device-map smoke test.
            self._run_device_preflight(trainer, raw_eval_samples=eval_samples)
        if self.args.debug_device_preflight_only:  # PORTED: allow preflight-only runs.
    ...
    def _run_device_preflight(self, trainer, raw_eval_samples=None):  # PORTED: multi-GPU forward_delta/eval smoke test.
        """
        ...
        def _reset_diff_counters_and_caches():  # PORTED: preflight must not change the first real LOZO step.
