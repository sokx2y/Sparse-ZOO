...
try:
    ...
    try:
        ...
        def deepspeed_init(*args, **kwargs):
            raise RuntimeError(
                "DeepSpeed is not available. Please do not pass --deepspeed, "
                "or install/configure DeepSpeed properly."
            )
...
is_torch_greater_or_equal_than_1_10 = version.parse(torch.__version__.split("+")[0]) >= version.parse("1.10")
...
class LowRankTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        # old copied Trainer code expects these attributes.
        if not hasattr(self, "sharded_ddp"):
            self.sharded_ddp = None
        if not hasattr(self, "fsdp"):
            self.fsdp = None
        if not hasattr(self.args, "sharded_ddp"):
            self.args.sharded_ddp = []
        if not hasattr(self.args, "fsdp"):
            self.args.fsdp = []
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        elif self.args.should_save and not getattr(self, "deepspeed", None):
    ...
    def _inner_training_loop(
        ...
        self.do_grad_scaling = getattr(self.args, "do_grad_scaling", False)
        ...
        # self._patch_stateful_callbacks()
...
# 
    ...
    def _debug_phase1_compare_step(self, model, inputs):
        self.lowrank_zo_perturb_parameters(scaling_factor=1)
        loss_plus_explicit = self.zo_forward(model, inputs, with_delta=False)
        loss_plus_fd, loss_minus_fd = self.zo_forward(model, inputs, with_delta=True)

        self.lowrank_zo_perturb_parameters(scaling_factor=-2)
        loss_minus_explicit = self.zo_forward(model, inputs, with_delta=False)

        self.lowrank_zo_perturb_parameters(scaling_factor=1)

        projected_grad_explicit = (loss_plus_explicit - loss_minus_explicit) / (2 * self.args.zo_eps)
        projected_grad_fd = (loss_plus_fd - loss_minus_fd) / (2 * self.args.zo_eps)

        loss_plus_explicit_f = float(loss_plus_explicit.detach().cpu())
        loss_plus_fd_f = float(loss_plus_fd.detach().cpu())
        abs_loss_plus = abs(loss_plus_explicit_f - loss_plus_fd_f)

        loss_minus_explicit_f = float(loss_minus_explicit.detach().cpu())
        loss_minus_fd_f = float(loss_minus_fd.detach().cpu())
        abs_loss_minus = abs(loss_minus_explicit_f - loss_minus_fd_f)

        projected_grad_explicit_f = float(projected_grad_explicit.detach().cpu())
        projected_grad_fd_f = float(projected_grad_fd.detach().cpu())
        abs_projected_grad = abs(projected_grad_explicit_f - projected_grad_fd_f)
        rel_projected_grad = abs_projected_grad / max(abs(projected_grad_explicit_f), 1e-12)

        row = {
            "phase1_step": self.step,
            "zo_random_seed": int(self.zo_random_seed),
            "eps": float(self.args.zo_eps),
            "rank_r": int(self.args.rank_r),
            "step_interval": int(self.args.step_interval),
            "loss_plus_explicit": loss_plus_explicit_f,
            "loss_plus_fd": loss_plus_fd_f,
            "abs_loss_plus": abs_loss_plus,
            "loss_minus_explicit": loss_minus_explicit_f,
            "loss_minus_fd": loss_minus_fd_f,
            "abs_loss_minus": abs_loss_minus,
            "projected_grad_explicit": projected_grad_explicit_f,
            "projected_grad_fd": projected_grad_fd_f,
            "abs_projected_grad": abs_projected_grad,
            "rel_projected_grad": rel_projected_grad,
        }

        print(
            "[PHASE1] "
            f"step={row['phase1_step']} "
            f"seed={row['zo_random_seed']} "
            f"lp_exp={loss_plus_explicit_f:.12g} "
            f"lp_fd={loss_plus_fd_f:.12g} "
            f"abs_lp={abs_loss_plus:.12g} "
            f"lm_exp={loss_minus_explicit_f:.12g} "
            f"lm_fd={loss_minus_fd_f:.12g} "
            f"abs_lm={abs_loss_minus:.12g} "
            f"pg_exp={projected_grad_explicit_f:.12g} "
            f"pg_fd={projected_grad_fd_f:.12g} "
            f"abs_pg={abs_projected_grad:.12g} "
            f"rel_pg={rel_projected_grad:.12g}",
            flush=True,
        )

        jsonl_path = self.args.debug_phase1_jsonl
        ...
        self.projected_grad = projected_grad_fd_f
        return loss_plus_fd

    def _debug_phase1c_param_names(self):
        default_param_names = [
        ...
        configured_names = getattr(self.args, "debug_phase1c_param_names", "")
        if configured_names:
            return [name.strip() for name in configured_names.split(",") if name.strip()]
        return default_param_names

    def _debug_phase1c_effective_delta(self, model, inputs):
        param_names = self._debug_phase1c_param_names()
        params_by_name = dict(model.named_parameters())
        jsonl_path = self.args.debug_phase1c_jsonl
        jsonl_dir = os.path.dirname(jsonl_path)
        if jsonl_dir:
            os.makedirs(jsonl_dir, exist_ok=True)
        ...
        for name in param_names:
            ...
            if param is not None:
                w_plus[name] = param.data.detach().clone()
        ...
        for name in param_names:
            param = params_by_name.get(name)
            row = {
                "phase1c_step": int(getattr(self, "step", -1)),
                "zo_random_seed": int(self.zo_random_seed),
                "eps": float(self.args.zo_eps),
                "rank_r": int(self.args.rank_r),
                "step_interval": int(self.args.step_interval),
            ...
            if param is None or name not in w_plus:
                row.update({
                    "found": False,
                    "param_dtype": None,
                    "param_shape": None,
                    ...
                    "max_abs_error": None,
                    "mean_abs_error": None,
                    "effective_zero_ratio": None,
                })
                rows.append(row)
                print(f"[PHASE1C] name={name} missing", flush=True)
                continue

            effective_delta = param.data.detach().float() - w_plus[name].float()
            if param.data.ndim >= 2:
                if name not in self.u or name not in self.v:
                    row.update({
                        "found": True,
                        "param_dtype": str(param.data.dtype),
                        "param_shape": list(param.data.shape),
                        "ideal_norm": None,
                        "effective_norm": float(effective_delta.norm().cpu().item()),
                        "error_norm": None,
                        "rel_error": None,
                        "max_abs_error": None,
                        "mean_abs_error": None,
                        "effective_zero_ratio": float((effective_delta == 0).float().mean().cpu().item()),
                        "missing_cache": True,
                    })
                    rows.append(row)
                    print(f"[PHASE1C] name={name} dtype={param.data.dtype} missing_cache=True", flush=True)
                    continue
                u = self.u[name]
                v = self.v[name]
                ideal_delta = (-2 * self.args.zo_eps) * (u.float() @ v.float().t())
            else:
                if name not in self.z:
                    ...
                    continue
                z = self.z[name]
                ideal_delta = (-2 * self.args.zo_eps) * z.float()

            error = effective_delta - ideal_delta
            ideal_norm = float(ideal_delta.norm().cpu().item())
            effective_norm = float(effective_delta.norm().cpu().item())
            error_norm = float(error.norm().cpu().item())
            rel_error = error_norm / (ideal_norm + 1e-12)
            max_abs_error = float(error.abs().max().cpu().item())
            mean_abs_error = float(error.abs().mean().cpu().item())
            effective_zero_ratio = float((effective_delta == 0).float().mean().cpu().item())
            ...
            row.update({
                ...
                "effective_zero_ratio": effective_zero_ratio,
            })
            rows.append(row)
            print(
                "[PHASE1C] "
                f"name={name} "
                f"dtype={row['param_dtype']} "
                f"rel_error={rel_error:.12g} "
                f"error_norm={error_norm:.12g} "
                f"ideal_norm={ideal_norm:.12g} "
                f"effective_zero_ratio={effective_zero_ratio:.12g}",
                flush=True,
            )
    ...
    def lowrank_zo_step(self, model, inputs):
        ...
        if getattr(self.args, "debug_phase1_compare", False):
            return self._debug_phase1_compare_step(model, inputs)

        if getattr(self.args, "debug_phase1c_delta", False):
            return self._debug_phase1c_effective_delta(model, inputs)
