...
from utils import *
from LOZOtrainer_DEbug import LowRankTrainer
...
class OurArguments(TrainingArguments):
    ...
    debug_phase1_compare: bool = field(
        default=False,
        metadata={"help": "Compare explicit LOZO losses with forward_delta losses without updating parameters."}
    )

    debug_phase1_jsonl: str = field(
        default="phase1_compare.jsonl",
        metadata={"help": "JSONL path for Phase1 explicit-vs-forward_delta comparison rows."}
    )

    debug_phase1c_delta: bool = field(
        default=False,
        metadata={"help": "Compare ideal perturbation delta with stored effective fp delta for selected parameters."}
    )

    debug_phase1c_jsonl: str = field(
        default="phase1c_delta.jsonl",
        metadata={"help": "JSONL path for Phase1c effective-delta debug rows."}
    )

    debug_phase1c_param_names: str = field(
        default="",
        metadata={"help": "Comma-separated parameter names for Phase1c delta checks; empty uses Llama defaults."}
    )
