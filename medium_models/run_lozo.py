...
def _svd_lora_quant_settings(data_args):
    quant_format = (data_args.svd_lora_quant_format or "none").lower()
    if quant_format in {"none", "fp", "fp16", "fp32"}:
        return "none", "none"
    if quant_format in {"nvint4", "nvfp4", "nvfp8"}:
        return "nv", quant_format
    if quant_format in {"mxint4", "mxfp4", "mxfp8"}:
        return "mx", quant_format
    raise ValueError(
        f"Unsupported --svd_lora_quant_format={data_args.svd_lora_quant_format}. "
        "Expected none, nvint4, nvfp4, nvfp8, mxint4, mxfp4, or mxfp8."
    )


def _svd_lora_checkpoint_layer_names(state_dict, adapter_name):
    suffix = f".lora_A.{adapter_name}.weight"
    return {key[: -len(suffix)] for key in state_dict if isinstance(key, str) and key.endswith(suffix)}


def _linear_layer_names(model):
    return {name for name, module in model.named_modules() if isinstance(module, torch.nn.Linear)}


def _check_svd_lora_key_compatibility(model, state_dict, metadata, data_args):
    ckpt_model_type = metadata.get("export_model_type")
    ckpt_model_class = metadata.get("export_model_class") or metadata.get("model_class")
    if ckpt_model_type and ckpt_model_type != model.config.model_type:
        raise ValueError(
            f"SVD-LoRA checkpoint model_type mismatch: checkpoint={ckpt_model_type}, "
            f"runtime={model.config.model_type}"
        )
    if ckpt_model_class:
        logger.info("[SVD-LoRA] checkpoint export_model_class=%s", ckpt_model_class)

    checkpoint_layers = _svd_lora_checkpoint_layer_names(state_dict, data_args.svd_lora_adapter_name)
    runtime_layers = _linear_layer_names(model)
    matched = checkpoint_layers & runtime_layers
    if not matched:
        checkpoint_examples = sorted(checkpoint_layers)[:8]
        runtime_examples = sorted(runtime_layers)[:8]
        raise ValueError(
            "SVD-LoRA checkpoint keys do not match the current medium_model RoBERTa module names. "
            f"adapter_name={data_args.svd_lora_adapter_name}, "
            f"checkpoint_examples={checkpoint_examples}, runtime_examples={runtime_examples}. "
            "Regenerate with SVD-ZOO-Quant --model-class masked-lm for prompt/LM-head runs "
            "or --model-class sequence-classification for classifier runs."
        )
    missing_at_runtime = checkpoint_layers - runtime_layers
    logger.info(
        "[SVD-LoRA] checkpoint/runtime key overlap: matched=%d checkpoint_layers=%d runtime_linear_layers=%d",
        len(matched),
        len(checkpoint_layers),
        len(runtime_layers),
    )
    if missing_at_runtime:
        logger.warning(
            "[SVD-LoRA] %d checkpoint layers are not present in the runtime model; first examples=%s",
            len(missing_at_runtime),
            sorted(missing_at_runtime)[:8],
        )


def _prepare_svd_lora_compensation(model, tokenizer, data_args):
    if not data_args.svd_lora_compensation:
        return None, {}
    if not data_args.apply_forward_delta:
        raise ValueError("--svd_lora_compensation requires --apply_forward_delta")
    if model.config.model_type != "roberta":
        raise NotImplementedError(
            f"medium_model SVD-LoRA compensation is implemented for RoBERTa only, got model_type={model.config.model_type}"
        )
    if not data_args.svd_lora_checkpoint_path:
        raise ValueError(
            "SVD-LoRA compensation requires a checkpoint generated by "
            "SVD-ZOO-Quant/experiments/init_lora_from_svd.py"
        )
    if not data_args.svd_lora_act_scales_path:
        raise ValueError(
    ...
    _add_svd_zoo_to_path()
    from smoothquant.smooth import smooth_lm

    act_scales_path = Path(data_args.svd_lora_act_scales_path)
    ...
    smooth_lm(model, act_scales, data_args.svd_lora_smooth_alpha)

    checkpoint_path = Path(data_args.svd_lora_checkpoint_path)
    ...
    _check_svd_lora_key_compatibility(model, state_dict, metadata, data_args)

    ckpt_rank = metadata.get("rank")
    if ckpt_rank is not None and int(ckpt_rank) != int(data_args.svd_lora_rank):
        raise ValueError(
            f"SVD-LoRA rank mismatch: checkpoint rank={ckpt_rank}, "
            f"--svd_lora_rank={data_args.svd_lora_rank}"
        )
    mode = (metadata.get("compensation_mode") or "quant_error").lower()
    low_rank_input = (metadata.get("low_rank_input") or "").lower()
    arg_quant_format = (data_args.svd_lora_quant_format or "none").lower()
    logger.info("[SVD-LoRA] checkpoint compensation_mode=%s quant_format=%s", mode, metadata.get("quant_format"))
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    # SmoothQuant + SVD-LoRA residual/compensation path.
    ...
    svd_lora_rank: int = field(
        default=8,
        metadata={"help": "Rank of the offline SVD-LoRA compensation factors. Independent from LOZO perturbation rank."}
    )

    svd_lora_quant_format: str = field(
        default="nvfp8",
        metadata={"help": "Quantization format matching the offline SVD checkpoint: none, nvint4, nvfp4, nvfp8, mxint4, mxfp4, mxfp8."}
    )

    quant_format_wdx: str = field(
        default="",
        metadata={"help": "Optional extra weight quantization format used only for the SVD-LoRA forward_delta dx * W branch."}
    )
    ...
    appro_SVD: bool = field(
        ...
        metadata={
            "help": (
                ...
                "Default keeps the exact SVD path."
            )
        }
    )
...
def main():
    ...
    svd_lora_state_dict, svd_lora_metadata = _prepare_svd_lora_compensation(model, tokenizer, data_args)
    ...
    if data_args.apply_forward_delta:
        ...
        if model.config.model_type == "roberta":
            if data_args.svd_lora_compensation:
                from src.diff_fake_quant_SVD import (
                    QuantizeRobertaForLOZOSVD,
                    attach_svd_lora_to_diff_layers,
                )
                _, svd_base_quant_format = _svd_lora_quant_settings(data_args)
                QuantizeRobertaForLOZOSVD(
                    model=model,
                    mx_w_elem_format=data_args.mx_w_elem_format,
                    mx_a_elem_format=data_args.mx_a_elem_format,
                    ...
                    quant_format=data_args.svd_lora_quant_format,
                    quant_format_wdx=data_args.quant_format_wdx,
                    group_size=data_args.svd_lora_group_size,
                )
            else:
        ...
        elif data_args.svd_lora_compensation:
            raise NotImplementedError(
                f"medium_model SVD-LoRA compensation is implemented for RoBERTa only, got model_type={model.config.model_type}"
            )
        ...
        if data_args.svd_lora_compensation:
            if svd_lora_state_dict is None:
            ...
            lora_scaling = 1.0 if ckpt_lora_alpha is None else float(ckpt_lora_alpha) / max(int(data_args.svd_lora_rank), 1)
            attach_svd_lora_to_diff_layers(
                model,
                svd_lora_state_dict,
                adapter_name=data_args.svd_lora_adapter_name,
                svd_lora_rank=data_args.svd_lora_rank,
                trainable=True,
                freeze_base=True,
